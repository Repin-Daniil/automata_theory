...
inline NFA GetNfaFromGrammar(const Grammar& grammar) {
  auto [Vn, Vt, S, P] = grammar.GetFormalRepresentation();

  //FIXME проверить на отсутствие пустой цепочки

  std::unordered_set states(Vn.begin(), Vn.end());
